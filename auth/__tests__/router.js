...
describe('POST /api/auth/login', () => {
  ...
  beforeEach(async () => {
    ...
    await request(server).post(REGISTER_ENDPOINT).send(mockUser)
  })
  ...
  describe('user login failure: invalid credentials', () => {
    test('should return HTTP status code 401 with accompanying message', async () => {
      const invalidCredentialsMsg = 'The provided username and password do not match'
      const invalidCredentials = { username: 'cedric', password: 'anincorrectpassword' }
      const res = await request(server).post(LOGIN_ENDPOINT).send(invalidCredentials)
  ...
  describe('user login successful', () => {
    test('should return HTTP status code 200', async () => {
      const validCredentials = { username: 'cedric', password: '12345' }
      const res = await request(server).post(LOGIN_ENDPOINT).send(validCredentials)
